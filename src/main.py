...
print(annot_data['name'][0])
print(annot_data.head())

df_cols = ['filename','width','height','classs','xmin','ymin','xmax','ymax']
fastrcnn = pd.DataFrame(columns=df_cols)

fastrcnn['filename'] = annot_data.apply( lambda row: row['name'], axis=1)
fastrcnn = fastrcnn.assign(width=512)
fastrcnn = fastrcnn.assign(height=512)
fastrcnn = fastrcnn.assign(classs='plane')
fastrcnn['xmin'] = annot_data.apply( lambda row: int(row['bbox'][0]), axis=1)
fastrcnn['ymin'] = annot_data.apply( lambda row: int(row['bbox'][1]), axis=1)
fastrcnn['xmax'] = annot_data.apply( lambda row: int(row['bbox'][0]+row['bbox'][2]), axis=1)
fastrcnn['ymax'] = annot_data.apply( lambda row: int(row['bbox'][1]+row['bbox'][3]), axis=1)
fastrcnn.rename(columns={"classs": "class"}, inplace=True)

print(fastrcnn.head())
fastrcnn.to_csv('train_labels.csv')
